...
function perseverance() {
  return sheet(1.9, 1.38, (g, W, H) => {
    ...
    const cx = W / 2, cy = H * 0.37, r = H * 0.115;
    g.fillStyle = '#20262f';
    ...
    g.fillStyle = '#c3bfb3';
    g.textAlign = 'center';
    line(g, 'PERSEVERANCE', W / 2, H * 0.83, 46, W * 0.84, 'Georgia, serif');
    g.fillStyle = '#7f8794';
    line(g, 'The line has been static for forty minutes.',
      W / 2, H * 0.915, 19, W * 0.88, 'Georgia, serif');
    line(g, 'You have not.', W / 2, H * 0.965, 19, W * 0.88, 'Georgia, serif');
  }, 560);
}
...
function waterMeme() {
  return sheet(2.0, 1.45, (g, W, H) => {
    g.fillStyle = '#12171f';
    g.fillRect(0, 0, W, H);
    // a slump cone that has entirely given up
    g.fillStyle = '#8e7a5f';
    g.beginPath();
    g.moveTo(W * 0.2, H * 0.68);
    g.quadraticCurveTo(W * 0.5, H * 0.34, W * 0.8, H * 0.68);
    ...
    const shout = (text: string, y: number, size: number) => {
      let s = size;
      do {
        g.font = '700 ' + s + 'px Impact, "Arial Black", sans-serif';
        s -= 1;
      } while (g.measureText(text).width > W * 0.92 && s > 10);
      g.strokeText(text, W / 2, y);
      g.fillText(text, W / 2, y);
    };
    shout('ADD MORE WATER', H * 0.2, 60);
    shout('FAIL THE UCS', H * 0.93, 60);
  }, 560);
}
...
function shiftBoard() {
  return sheet(1.55, 1.15, (g, W, H) => {
    ...
    g.textAlign = 'left';
    g.fillStyle = '#1b3d75';
    line(g, 'NIGHT SHIFT', W * 0.08, H * 0.16, 30, W * 0.6, 'Comic Sans MS, cursive');
    g.fillStyle = '#20252c';
    const items = [
      'fill 14-2 N',
      'DO NOT plug the line',
      'order balls BEFORE 20%',
      'slump 110-130 (Boger!)',
    ];
    items.forEach((it, i) => {
      g.fillStyle = i === 1 ? '#8e2c20' : '#20252c';
      line(g, (i === 1 ? '✖ ' : '✓ ') + it,
        W * 0.08, H * (0.33 + i * 0.145), 24, W * 0.86, 'Comic Sans MS, cursive');
    });
    g.fillStyle = '#1a5c39';
    line(g, 'whoever left the cone in the doorway - it was not funny',
      W * 0.08, H * 0.94, 15, W * 0.86, 'Comic Sans MS, cursive');
...
export class RoomDecor {
  ...
  constructor() {
    ...
    const meme = waterMeme();
    meme.mesh.position.set(-2.1, 3.56, 0);
    east.add(meme.mesh);
