...
/**
 * The five stations, in the order the ore meets them: what each one's sliders
...
export const fmt = {
  ...
  m: (v: number, dp = 1) => `${v.toFixed(dp)} m`,
};

const res = (s: Summary, node: string, key: string): number => s.results[node]?.[key] ?? Number.NaN;
const known = (v: number) => Number.isFinite(v);
